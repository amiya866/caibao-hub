...
(function () {
  ...
  function darkChartBase() {
    return {
      ...
      textStyle: { color: "#7a7772", fontFamily: "PingFang SC, Microsoft YaHei, sans-serif" },
      tooltip: {
        trigger: "axis",
        backgroundColor: "#ffffff", borderColor: "#e6e4e0",
        textStyle: { color: "#1d1d1c", fontSize: 12 }
      },
      grid: { left: 60, right: 60, top: 50, bottom: 40 }
    };
  }
  var PALETTE = ["#9c5f2c", "#5b7a8c", "#b4453a", "#3fae63", "#8a6f9c", "#b06a32", "#4f8a7a",
    "#8c6b9c", "#7a8c5b", "#2456e0", "#7a8a9c", "#6b7280", "#6b8c8c", "#c08a54", "#a04a6e",
    "#7a9a3a", "#5a6fb0", "#a88a3a", "#4a8ab0", "#9a7ab0", "#7a9a3a"];
  ...
  function renderProductionSection(container, sec, commodityKey, secKey, defaultView) {
    ...
    function renderTable() {
      ...
      groups.forEach(function (g) {
        ...
        g.items.forEach(function (c) {
          var est = c.est ? '<sup style="color:#9c5f2c" title="含估算/推算：' + esc(c.est_note || "") + '">†</sup>' : "";
          var label = isGroup && c.project
            ? esc(c.project) + est
            : esc(c.name) + est + (c.project ? ' <span style="color:#7a7772">· ' + esc(c.project) + "</span>" : "");
    ...
    function renderChart() {
        ...
        return {
          ...
          data: ps.map(function (p) {
            ...
            if (v !== null && c.est_q && c.est_q[p]) {
              return { value: v, itemStyle: { opacity: 0.45, borderColor: "#9c5f2c", borderWidth: 1, borderType: "dashed" } };
            }
            return v;
          })
        };
      ...
      series.push({
        ...
        lineStyle: { color: "#9c5f2c", width: 2, type: "dashed" },
        itemStyle: { color: "#9c5f2c" },
        data: ps.map(function (p) {
      ...
      var opt = Object.assign(darkChartBase(), {
        legend: { type: "scroll", top: 4, textStyle: { color: "#7a7772", fontSize: 11 } },
        tooltip: {
          trigger: "axis",
          backgroundColor: "#ffffff", borderColor: "#e6e4e0",
          textStyle: { color: "#1d1d1c", fontSize: 12 },
        ...
        xAxis: { type: "category", data: ps, axisLine: { lineStyle: { color: "#e6e4e0" } } },
        yAxis: [
          { type: "value", name: sec.unit, splitLine: { lineStyle: { color: "#efedea" } } },
          {
  ...
  function renderCompanyCards(container, commodity, section) {
    ...
    section.companies.forEach(function (c) {
      ...
      var headHtml = '<div class="co-head"><span class="co-name">' + esc(c.name) +
        (c.est ? '<sup style="color:#9c5f2c">†</sup>' : "") +
        (c.project ? ' <span style="font-weight:400;color:#a8a49e;font-size:12px">· ' + esc(c.project) + "</span>" : "") +
      ...
      if (g) {
        ...
        if (g.completed !== null) {
          progHtml = '<div class="co-line">2026 已完成 <b>' + fmtNum(g.completed) + "</b>" +
            ...
            (g.note ? ' <span style="color:#7a7772">（' + esc(g.note) + "）</span>" : "") + "</div>";
        } else if (g.status) {
      ...
      rgroups.forEach(function (g) {
        ...
        if (isGroup) {  // 集团小计父行：指引/已完成/年化求和，完成度按合计中枢重算
          ...
          html += '<tr class="grp-row"><td>' + esc(g.items[0].section) + "</td><td><b>" + esc(g.name) + "</b>" +
            ...
            '<td class="flat">-</td><td style="color:#7a7772">集团小计</td></tr>';
        }
        g.items.forEach(function (r) {
          ...
          html += "<tr><td>" + esc(r.section) + "</td><td" + (isGroup ? ' class="child-cell"' : "") + ">" + nameCell + "</td>" +
            ...
            '<td style="text-align:left;font-family:inherit;white-space:normal;min-width:150px;color:#2456e0">' + esc(r.fy2027 || "-") + "</td>" +
            '<td style="text-align:left;font-family:inherit;color:#7a7772">' + esc(r.note || "") + "</td></tr>";
        });
      });
    ...
    requestAnimationFrame(function () {
      ...
      tooltip: {
        ...
        formatter: function (p) {
          ...
          return "<b>" + esc(it.name) + "</b><br/>" + esc(labelFn(it)) +
            (it.note ? "<br/><span style='color:#a8a49e'>" + esc(it.note) + "</span>" : "") +
            (it.raw ? "<br/><span style='color:#a8a49e'>原文：" + esc(it.raw) + "</span>" : "") +
            (it.est ? "<br/><span style='color:#9c5f2c'>估算/折算值</span>" : "");
        }
      },
      xAxis: { type: "value", name: cf.currency || "US$/t", splitLine: { lineStyle: { color: "#efedea" } } },
      yAxis: {
        type: "category",
        data: cf.items.map(function (it) { return it.name; }),
        axisLine: { lineStyle: { color: "#e6e4e0" } },
      ...
      series: [{
        ...
        data: cf.items.map(function (it, i) {
          return {
            value: it[valueKey],
            itemStyle: { color: it.est ? "#9c5f2c" : PALETTE[i % PALETTE.length] }
          };
        }),
        label: {
          show: true, position: "right", color: "#1d1d1c",
    ...
    if (commodity.costs) {
      ...
      function lineChart(title, seriesDefs, currency) {
        ...
        requestAnimationFrame(function () {
          ...
          var inst = echarts.init(div, null, { renderer: "svg" });
          inst.setOption(Object.assign(darkChartBase(), {
          legend: { top: 4, textStyle: { color: "#7a7772", fontSize: 11 } },
          xAxis: { type: "category", data: seriesDefs[0].points.map(function (p) { return p.q; }), axisLabel: { rotate: 45 } },
          yAxis: { type: "value", name: currency, splitLine: { lineStyle: { color: "#efedea" } } },
  ...
  function renderCalendarTable(entries, showCommodity) {
    ...
    entries.forEach(function (e) {
      ...
      var badge = e.status === "待披露"
        ...
        : (e.status === "已入库"
          ? '<span class="badge instore">已入库</span>'
          : (e.status === "已核实·无季度数据"
            ? '<span class="badge done">已核实·无季度数据</span>'
            : '<span class="badge todo">' + esc(e.status || "已披露待核") + "</span>"));
  ...
  var NEWS_BADGE_CLASS = { "锡": "c-tin", "锌": "c-zinc", "铝": "c-alu", "宏观": "c-macro",
    "铜": "c-copper", "镍": "c-nickel", "铅": "c-lead", "锂": "c-lithium", "硅": "c-silicon" };
  var NEWS_IMPACT = {
  ...
  /* ================= 区块：近3个月供应扰动梳理 ================= */
  function renderDisruptions(container, commodity, titleOverride) {
    var items = commodity.disruptions || [];
    if (!items.length) return;  // 无数据不渲染该区块
    var section = el("div", "section");
    var head = el("div", "section-head");
    head.appendChild(el("div", "section-title", titleOverride || "近 3 个月供应扰动梳理"));
    head.appendChild(el("div", "section-sub",
  ...
  function renderReview(container, commodity) {
    ...
    if (rv.n_guide) {
      ...
      html += '<div class="co-line"><b>2026 指引进度</b>（' + rv.n_guide + " 家有数值指引）：" +
        '<span style="color:#3fae63">超出 ' + g["超出"] + "</span> · " +
        '<span style="color:#9c5f2c">符合 ' + g["符合"] + "</span> · " +
        '<span style="color:#b4453a">不及 ' + g["不及"] + "</span>（详见下方指引表）</div>";
    }
  ...
  /* ================= 页面：工作区聚合页（信息速递 / 供应扰动 / 披露日历） ================= */
  function renderNewsPage(main) {
    // 全品种新闻流（带品种徽标 + impact 红黄绿标），复用区块渲染、不限条数
    renderNewsSection(main, window.SITE_DATA.news || []);
  }
  function renderDisruptionsPage(main) {
    // 全品种扰动表聚合（减红增绿），逐品种分块
    var any = false;
    window.SITE_DATA.commodities.forEach(function (c) {
      if (!c.disruptions || !c.disruptions.length) return;
      any = true;
      renderDisruptions(main, c, c.name + " · 近 3 个月供应扰动");
    });
    if (!any) {
      var p = el("div", "panel");
      p.textContent = "暂无供应扰动记录。";
      main.appendChild(p);
    }
  }
  function renderCalendarPage(main) {
    // 全品种披露日历聚合（按日期升序，带品种列与倒计时）
    var allCal = [];
    window.SITE_DATA.commodities.forEach(function (c) { allCal = allCal.concat(c.calendar || []); });
    allCal.sort(function (a, b) { return a.date < b.date ? -1 : 1; });
    var section = el("div", "section");
    var head = el("div", "section-head");
    head.appendChild(el("div", "section-title", "披露日历"));
    head.appendChild(el("div", "section-sub", allCal.length + " 条 · 全部品种 · 按日期升序"));
    section.appendChild(head);
    var panel = el("div", "panel");
    var wrap = el("div", "table-wrap");
    wrap.innerHTML = renderCalendarTable(allCal, true);
    panel.appendChild(wrap);
    section.appendChild(panel);
  ...
  function start() {
    ...
    var pages = [{ key: "overview", name: "总览" },
      { key: "_news", name: "信息速递" },
      { key: "_dis", name: "供应扰动" },
      { key: "_cal", name: "披露日历" }].concat(
      DATA.commodities.map(function (c) { return { key: c.key, name: c.name, commodity: c }; })
    );
    var TICKER = { copper: "CU", aluminum: "AL", lead: "PB", zinc: "ZN",
      nickel: "NI", tin: "SN", lithium: "LC", silicon: "SI" };
    function show(key) {
      Array.prototype.forEach.call(tabs.children, function (t) {
        if (!t.dataset.key) return;  // 跳过侧栏分组标签
        t.className = "tab" + (TICKER[t.dataset.key] ? " c-" + t.dataset.key : "") +
          (t.dataset.key === key ? " active" : "");
      });
      main.innerHTML = "";
      if (key === "overview") renderOverview(main);
      else if (key === "_news") renderNewsPage(main);
      else if (key === "_dis") renderDisruptionsPage(main);
      else if (key === "_cal") renderCalendarPage(main);
      else {
    ...
    pages.forEach(function (p) {
      if (p.commodity && p.key === DATA.commodities[0].key) {
        tabs.appendChild(el("div", "side-sec", "品种工作区"));
      }
      var t = el("div", "tab" + (TICKER[p.key] ? " c-" + p.key : ""));
      if (TICKER[p.key]) t.innerHTML = '<span class="tk">' + TICKER[p.key] + "</span>" + esc(p.name);
    ...
    /* 顶栏搜索：品种名/代码 → 切页签；公司名 → 跳品种页并高亮该公司卡片/表格行 */
    var qInput = document.getElementById("q");
    function highlightCompany(name) {
      var target = null, i;
      var nmL = name.toLowerCase();
      var cardNames = main.querySelectorAll(".co-card .co-name");
      for (i = 0; i < cardNames.length; i++) {
        if (cardNames[i].textContent.toLowerCase().indexOf(nmL) >= 0) {
          target = cardNames[i].closest(".co-card"); break;
        }
      }
      if (!target) {
        var rows = main.querySelectorAll(".data-table tbody tr");
        for (i = 0; i < rows.length; i++) {
          var cell = rows[i].querySelector("td");
          if (cell && cell.textContent.toLowerCase().indexOf(nmL) >= 0) { target = rows[i]; break; }
        }
      }
      if (!target) return;
      var sec = target.closest(".section.collapsed");
      if (sec) sec.classList.remove("collapsed");
      target.scrollIntoView({ behavior: "smooth", block: "center" });
      target.classList.add("hit-glow");
      setTimeout(function () { target.classList.remove("hit-glow"); }, 2600);
    }
    function doSearch(raw) {
      var q = (raw || "").trim().toLowerCase();
      if (!q) return;
      var c0 = DATA.commodities.filter(function (c) {
        return c.name.toLowerCase().indexOf(q) >= 0 || c.key.toLowerCase() === q ||
          (TICKER[c.key] || "").toLowerCase() === q;
      })[0];
      if (c0) { show(c0.key); return; }
      for (var i = 0; i < DATA.commodities.length; i++) {
        var secs = DATA.commodities[i].sections || [];
        for (var j = 0; j < secs.length; j++) {
          var comps = secs[j].companies || [];
          for (var k = 0; k < comps.length; k++) {
            var nm = (comps[k].name + " " + (comps[k].project || "")).toLowerCase();
            if (nm.indexOf(q) >= 0) {
              show(DATA.commodities[i].key);
              highlightCompany(comps[k].name);
              return;
            }
          }
        }
      }
      var box = qInput.parentElement;
      box.classList.add("q-miss");
      setTimeout(function () { box.classList.remove("q-miss"); }, 1200);
    }
    if (qInput) {
