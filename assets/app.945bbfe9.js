  ...
  function start() {
    ...
    function show(key) {
      Array.prototype.forEach.call(tabs.children, function (t) {
        ...
        t.className = "tab" + (t.dataset.key === "overview" ? "" : " c-" + t.dataset.key) +
    ...
    pages.forEach(function (p, pi) {
      if (pi === 1) tabs.appendChild(el("div", "side-sec", "品种工作区"));
      var t = el("div", "tab" + (p.key === "overview" ? "" : " c-" + p.key));
      if (p.key === "overview") t.textContent = p.name;
      else t.innerHTML = '<span class="tk">' + (TICKER[p.key] || "") + "</span>" + esc(p.name);
      t.dataset.key = p.key;
